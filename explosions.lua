...
function explosions.load()
	for i, type in pairs(explosions.types) do
		...
		for i = 1, 5 do
			explosions.images[type][i] = love.graphics.newImage('img/explosions/explosion-' .. type .. (i - 1) .. '.png')
			explosions.images[type][i]:setFilter('nearest', 'nearest')
		end
	end
end

function explosions.spawn(target, blue, big, gray, transparent)
	local explosion = {
		currentTexture = 1,
		x = target.x,
		y = target.y,
	...
	if blue then explosion.prefix = 'blue'
	elseif gray then explosion.prefix = 'gray' end
...
function explosions.update()
	...
	for i = 1, #explosions.explosions do
		...
		if explosion then --???????
			...
			if explosion.clock == interval then explosion.currentTexture = 2
			elseif explosion.clock == interval * 2 then explosion.currentTexture = 3
			elseif explosion.clock == interval * 3 then explosion.currentTexture = 4
			elseif explosion.clock == interval * 4 then explosion.currentTexture = 5
			elseif explosion.clock >= interval * 5 then table.remove(explosions.explosions, i) end
		end
	end
end
...
function explosions.draw()
	for i = 1, #explosions.explosions do
		...
		if explosion.transparent then love.graphics.setStencilTest('greater', 0) end
		...
		if explosion.transparent then love.graphics.setStencilTest() end
	end
end
